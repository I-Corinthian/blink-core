...
// Not ideal - a modified copy of the options from blink-cli 
std::string HelpMessageCli() 
{
     ...
     strUsage += HelpMessageOpt( "-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BLINK_CONF_FILENAME));
     ...
     strUsage += HelpMessageOpt("-rpcwallet=<walletname>",
         _("Send RPC for non-default wallet on RPC server (argument is wallet "
           "filename in blinkd directory, required if blinkd runs with "
           "multiple wallets)"));
                                            ...
                                            RandomIntGenerator & random_int_func)
{
    ...
    while ((GetTime() < start + searchDuration) && !found)
    {
        ...
        // and the block will be rejected.  So do not advance time (let it be advanced by blinkd every time we
        // request a new block).
    ...
    while (0 != nblocks)
    {
        ...
        if (strPrint != "")
        {
            ...
            else
            {
                ...
                // The result is sent to blinkd above when the loop gets to it.
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        std::string appname("blink-miner");
        std::string usage = "\n" + _("Usage:") + "\n" + "  " + appname + " [options] " + "\n";
