 ...
 *
 * This is the developer documentation of Blink SV
 * (https://blinksv.io/). Blink SV is a client for the digital
 * currency called Blink SV, which enables
 * instant payments to anyone, anywhere in the world. Blink SV uses
...
bool AppInit(int argc, char *argv[]) {
    ...
    try {
        ...
        {
            fprintf(stderr, "Mandatory consensus parameter is not set. In order to start blinkd you must set the "
                            "following consensus parameters: \"excessiveblocksize\" and "
                            "\"maxstackmemoryusageconsensus\". In order to start blinkd with no limits you can set "
                            ...
                            "https://blinksv.io/choosing-consensus-settings/");
            return false;
        }
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "blink:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "blinkd anymore. Use the blink-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for blinkd
...
#if HAVE_DECL_DAEMON
            fprintf(stdout, "Blink server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect blinkd signal handlers
    noui_connect();
