...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total blink "
...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given blink address.\n"
            "\nArguments:\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Blink address and we have full public key:
        if (pwallet) {
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are blink addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) blink address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "blink-cli. There is no server-side difference.");
    }
