...
inline int set_error(blinkconsensus_error *ret,
                     blinkconsensus_error serror) {
    if (ret) *ret = serror;
    return 0;
}
...
static bool verify_flags(unsigned int flags) {
    return (flags & ~(blinkconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
...
static int verify_script(const CScriptConfig& config, const uint8_t* scriptPubKey,
                         ...
                         blinkconsensus_error* err) {
    if (!verify_flags(flags)) {
        return blinkconsensus_ERR_INVALID_FLAGS;
    }
    try {
        TxInputStream stream(SER_NETWORK, PROTOCOL_VERSION, txTo, txToLen);
        CTransaction tx(deserialize, stream);
        if (nIn >= tx.vin.size())
            return set_error(err, blinkconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, blinkconsensus_ERR_TX_SIZE_MISMATCH);

        // Regardless of the verification result, the tx did not error.
        set_error(err, blinkconsensus_ERR_OK);
        ...
        // Error deserializing
        return set_error(err, blinkconsensus_ERR_TX_DESERIALIZE);
    }
}

int blinkconsensus_verify_script_with_amount(
...
int blinkconsensus_verify_script(const CScriptConfig& config,
    ...
    if (flags & blinkconsensus_SCRIPT_ENABLE_SIGHASH_FORKID ||
        flags & blinkconsensus_SCRIPT_FLAGS_VERIFY_WITNESS_DEPRECATED) {
        return set_error(err, blinkconsensus_ERR_AMOUNT_REQUIRED);
    }
...
unsigned int blinkconsensus_version() {
    // Just use the API version for now
    return BLINKCONSENSUS_API_VER;
}
