...
'''
test that blocks are sent via authenticated connection
...
class AuthConnTestReputation(BlinkTestFramework):
    ...
    def run_test(self):
        ...
        connect_nodes_bi(self.nodes, 0, 1)
        assert (not self.isAuthenticated(0))
        assert (self.isAuthenticated(1))  # node0 good and authenticated after reconnection

        # m = 1 and n = 2 for node0
        # m = 0 and n = 2 for node1
        # following function increments block height by 2
        self.one_test(allKeys1, http_conns, aliases, nodenum=1)
        # m = 1, and n = 4 for node0
        # m = 1, and n = 4 for node1
        # at this point m of n condition is met for both nodes but we need to reconnect
        # to get the bidrectional authenticated status

        assert (not self.isAuthenticated(0))  # node1 reputation now good but need to reconnect
        assert (self.isAuthenticated(1))

        # connection is now authenticated in both directions because we reconnected
        # after meeting thee m of n condition for both nodes
        disconnect_nodes_bi(self.nodes, 0, 1)
        connect_nodes_bi(self.nodes, 0, 1)
