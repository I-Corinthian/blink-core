    ...
    def submit_block_catch_error(self, node_index, block):
        ...
        try:
            ...
            if sys.version_info[0] == 3 and sys.version_info[1] < 5 and e.line == "''":
                self.log.debug(
                    "node %d submitblock raised exception: %s", node_index, e)
                return False
            else:
                raise
            ...
            self.log.debug(
                "node %d submitblock raised OSError exception: errno=%s", node_index, e.errno)
        ...
        for i in range(40):
            ...
            if random_height > starting_tip_height:
                ...
                if random.random() < 1.0 / (current_height + 4 - random_height):
                    self.log.debug(
                        "Invalidating block at height %d", random_height)
