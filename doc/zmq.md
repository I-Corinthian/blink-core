...
The Blink SV daemon can be configured to act as a trusted "border
router", implementing the blink wire protocol and relay, making
...
The ZeroMQ feature in Blink SV requires ZeroMQ API version 4.3.2 or
...
during the *configure* step of building blinkd:
...
For instance:
    ...
    $ blinkd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/blinkd.tx.raw

These options can also be provided in blink.conf.
...
From the perspective of blinkd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into Blink SV directly. Furthermore, no information is
...
using. Blink SV appends an up-counting sequence number to each
